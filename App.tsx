...
const App: React.FC = () => {
  ...
  const t: any = {
    TR: { search: "Ara...", explore: "KEŞFET", collections: "ÖZEL KOLEKSİYONLAR", banner: bannerTR, contactTitle: "MÜZİĞİNİ PAYLAŞ", contactSub: '"Tozlu raflarda unutulmuş bir kayıt mı var?"', contactDesc: "Kültürel mirasımızı birlikte ilmek ilmek işleyelim. Elinizdeki yöresel kayıtları bize ulaştırın.", waBtn: "WHATSAPP", waHatti: "WHATSAPP HATTI", waNum: "0505 225 06 55", adrTitle: "ADRES", adrText: "Yeşilbağlar Mah. 637/33 Sok. No: 25 Buca/İzmir", mailTitle: "E-POSTA", topTracks: "EN ÇOK BEĞENİLENLER", catNames: { "Patnos Türküleri": "Patnos Türküleri", "Patnoslu Sanatçılar": "Patnoslu Sanatçılar", "Dengbêjler": "Dengbêjler", "Sizden Gelenler": "Sizden Gelenler" } },
    KU: { search: "Bigere...", explore: "KEŞIF BIKE", collections: "KOLEKSIYONÊN TAYBET", banner: bannerKU, contactTitle: "MUZÎKA XWE PARVE BIKE", contactSub: '"Ma qeydeke ji bîr kiriye heye?"', contactDesc: "Werin em mîrateya xwe ya çandî bi hev re biparêzin. Qeydên xwe ji me re bişînin.", waBtn: "WHATSAPP", waHatti: "XETA WHATSAPPÊ", waNum: "0505 225 06 55", adrTitle: "ADRES", adrText: "Taxa Yeşilbağlar. 637/33 Sok. No: 25 Buca/Îzmîr", mailTitle: "E-POSTA", topTracks: "YÊN HERÎ ZÊDE HATINE BEĞENÎ KIRIN", catNames: { "Patnos Türküleri": "Stranên Panosê", "Patnoslu Sanatçılar": "Hunermendên Panosî", "Dengbêjler": "Dengbêj", "Sizden Gelenler": "Ji We Hatiye" } }
  };
          ...
          {activeTab === 'home' && (
            <div className="animate-in fade-in duration-700">
               <div className="mb-8 rounded-[3.5rem] relative overflow-hidden h-[300px] flex items-center shadow-2xl">
                ...
                <div className="relative z-10 p-16"><h2 className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter leading-none">{t[lang].banner}</h2></div>
              </div>
                ...
                {displaySongs.map((song: any) => (
                  <div key={song.id} className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-transparent hover:border-white/10 transition-all group">
                    ...
                    <div className="flex items-center space-x-6 pr-4">
                         <div className="hidden md:block text-[10px] font-mono text-neutral-600">
                           <DurationDisplay url={song.url} />
                         </div>
          ...
          {activeTab === 'contact' && (
             <div className="max-w-5xl mx-auto py-10 space-y-8 animate-in slide-in-from-bottom-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                   <div className="bg-neutral-900/80 p-10 rounded-[2.5rem] text-center shadow-xl"><h3 className="text-amber-500 font-black mb-4 uppercase text-[10px] tracking-widest">{t[lang].adrTitle}</h3><p className="text-[11px] italic text-neutral-300 font-bold">{t[lang].adrText}</p></div>
                   <div className="bg-neutral-900/80 p-10 rounded-[2.5rem] text-center shadow-xl"><h3 className="text-amber-500 font-black mb-4 uppercase text-[10px] tracking-widest">{t[lang].waHatti}</h3><p className="text-lg italic text-neutral-100 font-black">{t[lang].waNum}</p></div>
                   <div className="bg-neutral-900/80 p-10 rounded-[2.5rem] text-center shadow-xl"><h3 className="text-amber-500 font-black mb-4 uppercase text-[10px] tracking-widest">{t[lang].mailTitle}</h3><p className="text-sm italic text-neutral-100 font-black">patnosumuz@gmail.com</p></div>
                </div>
             </div>
          )}
                    ...
                    {songs.map((song: any) => (
                      <div key={song.id} className="flex items-center justify-between p-4 bg-black/40 rounded-2xl border border-white/5">
                        ...
                        <div className="flex space-x-2">
                           <button onClick={() => { const n = prompt("Ad:", song.title); if(n) setSongs(songs.map(s => s.id === song.id ? {...s, title: n} : s)) }} className="bg-amber-500/10 text-amber-500 px-4 py-2 rounded-xl text-[10px] font-black">DÜZENLE</button>
                           <button onClick={() => { if(window.confirm("Silinsin mi?")) setSongs(songs.filter((s:any) => s.id !== song.id)) }} className="bg-red-500/10 text-red-500 px-4 py-2 rounded-xl text-[10px] font-black">SİL</button>
                        </div>
                      </div>
                    ))}
                  </div>
