  ...
  return (
    <div className="flex h-screen bg-neutral-950 text-white overflow-hidden font-['Outfit']">
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        <header className="h-20 flex items-center justify-between px-6 md:px-10 border-b border-white/10 shrink-0 bg-neutral-950">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 pb-48">
          {activeTab === 'home' && (
            <section className="animate-in fade-in duration-500">
              {/* SÜPHAN DAĞI BANNER - KALICI ÇAKILI GÖRSEL */}
              <div className="mb-10 rounded-[2.5rem] relative overflow-hidden min-h-[300px] flex items-end shadow-2xl border border-white/5 bg-black">
                ...
                <div className="relative z-10 p-8 md:p-12">
                  <span className="bg-amber-500 text-black text-[10px] font-black px-3 py-1 rounded-full mb-4 inline-block uppercase tracking-widest italic">Süphan Dağı</span>
                  <h2 className="text-4xl md:text-6xl font-black tracking-tighter leading-tight drop-shadow-2xl">{bannerText}</h2>
                </div>
              </div>

              {/* MÜZİK LİSTESİ VE İKONLAR */}
                ...
                {filteredSongs.map((song, idx) => (
                  <div key={song.id} onClick={() => { setCurrentSong(song); setIsPlaying(true); }} className={`group flex items-center justify-between p-4 rounded-2xl transition-all cursor-pointer hover:bg-white/5 ${currentSong?.id === song.id ? 'bg-amber-500/10' : ''}`}>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); handleDownload(song); }} className="p-2 text-neutral-500 hover:text-amber-500 transition-all">
          ...
          {activeTab === 'contact' && (
            <div className="max-w-4xl mx-auto py-10 space-y-10">
              <h2 className="text-3xl font-black text-amber-500 uppercase text-center italic underline underline-offset-8">İLETİŞİM BİLGİLERİ</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <a href="https://wa.me/905052250655" target="_blank" rel="noreferrer" className="bg-white/5 p-6 rounded-[2rem] border border-white/10 flex flex-col items-center text-center space-y-4 hover:bg-green-500/10 transition-all">
                  <div className="w-14 h-14 rounded-2xl bg-green-500/20 flex items-center justify-center"><i className="fab fa-whatsapp text-2xl text-green-500"></i></div>
                  <div><h4 className="font-black text-amber-500 text-[10px] uppercase mb-1 tracking-widest">WhatsApp</h4><p className="text-white font-bold text-sm">0505 225 06 55</p></div>
                </a>
                <a href="mailto:patnosumuz@gmail.com" className="bg-white/5 p-6 rounded-[2rem] border border-white/10 flex flex-col items-center text-center space-y-4 hover:bg-blue-500/10 transition-all">
                  <div className="w-14 h-14 rounded-2xl bg-blue-500/20 flex items-center justify-center"><i className="fas fa-envelope text-2xl text-blue-500"></i></div>
                  <div><h4 className="font-black text-amber-500 text-[10px] uppercase mb-1 tracking-widest">E-Posta</h4><p className="text-white font-bold text-xs">patnosumuz@gmail.com</p></div>
                </a>
                <div className="bg-white/5 p-6 rounded-[2rem] border border-white/10 flex flex-col items-center text-center space-y-4">
                  <div className="w-14 h-14 rounded-2xl bg-amber-500/20 flex items-center justify-center"><i className="fas fa-map-marker-alt text-2xl text-amber-500"></i></div>
                  <div><h4 className="font-black text-amber-500 text-[10px] uppercase mb-1 tracking-widest">Adres</h4><p className="text-white font-bold text-[10px]">Yeşilbağlar Mah. 637/33 Sok. No: 25 Buca/İzmir</p></div>
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'admin' && isAdmin && (
            <div className="max-w-3xl mx-auto space-y-10 py-10">
              <div className="p-8 bg-white/5 rounded-[2.5rem] border border-white/10">
                <h2 className="text-xl font-black mb-6 text-amber-500 uppercase italic">Banner Yazısını Değiştir</h2>
              ...
              <div className="p-8 bg-amber-500/10 rounded-[2.5rem] border border-amber-500/20 text-center">
                <h2 className="text-xl font-black mb-6 text-amber-500 uppercase tracking-widest italic">Müzik Yükleme Notu</h2>
                ...
                <a href="https://vercel.com/dashboard/stores" target="_blank" rel="noreferrer" className="inline-block bg-amber-500 text-black px-8 py-4 rounded-2xl font-black text-sm hover:bg-amber-600">VERCEL BLOB'A GİT</a>
              </div>
            </div>
          )}
        </div>
