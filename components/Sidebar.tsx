  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#121212] border-r border-white/5 transform transition-transform duration-300 md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="flex flex-col h-full p-6">
        ...
        {/* ERİŞİM PANELİ (PLAYER'IN ALTINDA KALMAMASI İÇİN YERİ DEĞİŞTİ) */}
        <div className="bg-neutral-900/40 p-5 rounded-[2rem] border border-white/5 shadow-inner">
          {!isAdmin ? (
            <form onSubmit={handleAdminLogin} className="space-y-3">
              <p className="text-[9px] text-neutral-500 font-black text-center uppercase tracking-widest mb-1">Sistem Erişimi</p>
              <input 
                type="password" 
                placeholder="Şifre..." 
                className="w-full bg-black/60 border border-white/10 rounded-xl px-4 py-2 text-[11px] text-white focus:border-amber-500 outline-none transition-all"
                value={adminPass}
                onChange={(e) => setAdminPass(e.target.value)}
              />
              <button className="w-full bg-amber-500 text-black text-[10px] font-black py-2.5 rounded-xl uppercase tracking-tighter hover:brightness-110 active:scale-95 transition-all">Giriş Yap</button>
            </form>
          ) : (
            <div className="space-y-4">
              <div className="flex items-center justify-center space-x-2 text-green-500">
                <span className="relative flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span></span>
                <p className="text-[10px] font-black uppercase tracking-widest italic">Yönetici Aktif</p>
              </div>
              <button onClick={() => { setIsAdmin(false); setActiveTab('home'); }} className="w-full bg-white/5 hover:bg-red-500/10 hover:text-red-500 text-neutral-500 text-[9px] font-black py-2 rounded-xl border border-white/5 transition-all">GÜVENLİ ÇIKIŞ 🚪</button>
            </div>
          )}
        </div>

        {/* PLAYER İÇİN ALT BOŞLUK */}
