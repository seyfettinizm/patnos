      ...
      <div className="flex flex-col h-full p-6 pb-44">
        ...
        <div className="flex flex-col items-center mb-10 border-b border-white/5 pb-6">
          ...
          <div className="text-center">
            <h1 className="text-[12px] font-black text-white uppercase tracking-tighter">İzmir Patnoslular</h1>
            <p className="text-[9px] font-bold text-amber-500 uppercase tracking-widest mt-1">Sosyal Yardımlaşma Derneği</p>
          </div>
        </div>
        
        <nav className="space-y-2 flex-1">
          <button onClick={() => { setActiveTab('home'); setIsOpen(false); }} className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl font-bold transition-all ${activeTab === 'home' ? 'bg-amber-500 text-black' : 'text-neutral-400 hover:bg-white/5'}`}>
            <i className="fas fa-home"></i>
            <span>Ana Sayfa</span>
          </button>

          <button onClick={() => { setActiveTab('contact'); setIsOpen(false); }} className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl font-bold transition-all ${activeTab === 'contact' ? 'bg-amber-500 text-black' : 'text-neutral-400 hover:bg-white/5'}`}>
            <i className="fas fa-envelope"></i>
            <span>İletişim</span>
          </button>

          {isAdmin && (
            <button onClick={() => { setActiveTab('admin'); setIsOpen(false); }} className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl mt-4 font-bold bg-red-500 text-white`}>
        ...
        {/* SİSTEM GİRİŞİ - PLAYER ALTINDA KALMAZ */}
        {!isAdmin && (
          <div className="bg-white/5 p-4 rounded-2xl border border-white/5 mt-auto mb-10">
            <form onSubmit={handleAdminLogin} className="space-y-3">
              <p className="text-[10px] text-neutral-500 uppercase font-black tracking-widest text-center italic">Sistem Erişimi</p>
              <input 
                type="password" 
                placeholder="Şifre..." 
