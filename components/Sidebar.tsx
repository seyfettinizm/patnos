    ...
    <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#121212] border-r border-white/5 transform transition-transform duration-300 md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="flex flex-col h-full p-6 pb-32">
        ...
        <div className="mb-10 flex items-center space-x-3">
          <div className="w-10 h-10 bg-amber-500 rounded-lg flex items-center justify-center font-black text-black shadow-lg">P</div>
          <div>
        ...
        <nav className="space-y-2 flex-1">
          <p className="text-[9px] text-neutral-600 font-black uppercase tracking-[0.2em] mb-4 ml-2">Menü</p>
        ...
        {/* GİRİŞ PANELİ (Player'ın üzerinde kalması için yukarı çekildi) */}
        <div className="mt-auto">
          {!isAdmin ? (
            <div className="bg-neutral-900/50 p-4 rounded-2xl border border-white/5">
              <form onSubmit={handleAdminLogin} className="space-y-3">
                <p className="text-[8px] text-neutral-500 font-black text-center uppercase tracking-widest">Erişim</p>
                <input 
                  type="password" 
                  placeholder="Şifre..." 
                  className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-[10px] text-white focus:border-amber-500 outline-none"
                  value={adminPass}
                  onChange={(e) => setAdminPass(e.target.value)}
                />
                <button className="w-full bg-amber-500 text-black text-[9px] font-black py-2 rounded-lg uppercase tracking-tighter">Giriş</button>
              </form>
            </div>
          ) : (
