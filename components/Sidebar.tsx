...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-black border-r border-white/10 transform transition-transform duration-300 md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="flex flex-col h-full p-6 pb-32"> {/* pb-32 eklendi: Müzikçalar altında kalmasın */}
        
        {/* LOGO ALANI */}
        <div className="flex flex-col items-center mb-10 border-b border-white/5 pb-6">
          <div className="w-24 h-24 mb-4 rounded-full overflow-hidden border-2 border-amber-500/50 shadow-xl bg-white">
            <img 
        ...
        <nav className="space-y-2 flex-1">
          ...
          {isAdmin && (
            <button onClick={() => setActiveTab('admin')} className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl font-bold mt-4 transition-all ${activeTab === 'admin' ? 'bg-red-500 text-white' : 'text-red-400 hover:bg-red-500/10'}`}>
        ...
        {/* GİRİŞ FORMU - Player'ın üstünde kalması için düzenlendi */}
        {!isAdmin && (
          <div className="bg-white/5 p-4 rounded-2xl border border-white/5 mt-auto mb-4">
            <form onSubmit={handleAdminLogin} className="space-y-3">
              ...
              />
              <button className="w-full bg-amber-500 text-black text-[10px] font-black py-2 rounded-lg hover:bg-amber-600 transition-all">
                GİRİŞ YAP
              </button>
            </form>
          </div>
