    ...
    <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-black border-r border-white/10 transform transition-transform duration-300 md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="flex flex-col h-full p-6 pb-44">
        ...
        <nav className="space-y-2 flex-1 font-bold">
          ...
          {isAdmin && (
            <button onClick={() => { setActiveTab('admin'); setIsOpen(false); }} className="w-full flex items-center space-x-3 px-4 py-3 rounded-xl mt-4 bg-red-600 text-white shadow-lg shadow-red-600/20">
        ...
        {!isAdmin && (
          <div className="bg-white/5 p-4 rounded-2xl border border-white/5 mt-auto mb-6">
            <form onSubmit={handleAdminLogin} className="space-y-3">
              <p className="text-[10px] text-neutral-500 uppercase font-black text-center italic mb-1">Erişim</p>
              <input 
                type="password" 
                placeholder="Şifre..." 
              ...
              />
              <button className="w-full bg-amber-500 text-black text-[10px] font-black py-2 rounded-lg hover:bg-amber-600 transition-colors uppercase tracking-widest">GİRİŞ</button>
            </form>
          </div>
        )}
      </div>
